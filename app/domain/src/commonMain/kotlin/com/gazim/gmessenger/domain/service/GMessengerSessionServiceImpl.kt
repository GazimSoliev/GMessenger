...
class GMessengerSessionServiceImpl(
    ...
    override suspend fun getMessages(chatModel: IChat, key: MessagePageKey?) = gMessengerService.getMessages(token, chatModel, key)

    override suspend fun editProfile(profileForm: ProfileForm) = gMessengerService.editProfile(token, profileForm)

    override suspend fun uploadProfilePhoto(type: String, bytes: ByteArray) = gMessengerService.uploadProfilePhoto(token, type, bytes)
