...
class AuthorizationServiceImpl(
    ...
    override suspend fun login(
        ...
        transaction {
            val userId = userRepository.findUserByLoginAndPassword(
                login = login,
                password = password,
            ) ?: return@transaction
            token = tokenRepository.insertAndGetToken(
                createdAt = createdAt,
                expiredAt = expiredAt,
                userId = userId
            )
    ...
    override suspend fun register(account: RegistrationForm): Boolean {
        ...
        val createdAt = Clock.System.now()
        transaction {
            val userId = userRepository.insertAndGetId(
